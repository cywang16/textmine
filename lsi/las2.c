...
char *argv[];
{
   ...
   double *r, *ritz, *bnd, *d, *tptr1, *uiptr;
   long nn, k, i, ui, id, ida, n, lanmax, maxprs, nnzero;
   long memory, vectors, size1, size2, count1, count2;
   char title[73], name[41], v[6];
   char in1[100], in2[100], out1[100], out2[100];
   FILE *fp_in1, *fp_in2;

   /*-- start: use the directory arg to build a complete path name --*/
   strcpy(in1, argv[1]);  strcpy(in2, argv[1]);
   strcpy(out1, argv[1]); strcpy(out2, argv[1]); 
   strcat(in1, "lap2");   strcat(in2, "matrix");
   strcat(out1, "lao2");  strcat(out2, "lav2"); 
   /*-- end: --*/
...
/*-- BEGIN code modification --*/
	  ...
	  write(fp_out2, (char *)&xv1[ida], size1);
          ...
          /*-- start: dump the u vectors */
          uiptr = &xv1[ida];
          for (ui = 0; ui < nrow; ui++)
           { fprintf (fp_out1, "%10.8f ", *uiptr); uiptr++; }
          fprintf(fp_out1, "\n");
          /*-- end: */
	  id += nn;
       }
       fprintf(fp_out1, "End of U:\n");
     /*  exetime += (timer() - t0); */
...
/*
       ...
       for (i = 0; i < nsig; i++)
		  ...
		  i + 1, d[i], bnd[i]);
*/
       fprintf(fp_out1, "Start of Sigma:\n");
       for (i = 0; i < nsig; i++) fprintf(fp_out1, "%15.10f ", d[i]);
       fprintf(fp_out1, "\nEnd of Sigma:\n");
       /*-- end: */
...
/*-- BEGIN code modification --*/
       fprintf(fp_out1, "Start of Sigma:\n");
       for (i = 1; i <= nsig; i++, k++) 
         fprintf(fp_out1, "%15.10f ", sqrt(ritz[k]));
       fprintf(fp_out1, "\nEnd of Sigma:\n");
/*-- END code modification --*/
       /*-- end: */
   }
		      ...
		      long nnzero) 
{
   ...
   if (error_index) fprintf(fp_out1, "%d%s\n", error_index, error[error_index]);
   return(error_index);
}
...
extern long nrow, ierr, j, fp_out2, neig, nsig;
extern double *xv1;
...
/*-- BEGIN code modification --*/
   ...
   for (k = 0; k < js; k++) {
	 ...
	 for (i = 0; i < n; i++) w1[i] = 0.0;
      ...
      /*-- start: dump the V vectors */
      for (i = 0; i < n; i++) fprintf (fp_out1, "%10.5f ", w1[i]); 
      fprintf(fp_out1, "\n"); 
      /*-- end: */
...

{
   ...
   for (id = 0; id < 3; id++) {
      if (id > 0 || j > 0 || rnm2 == 0) 
	 for (i = 0; i < n; i++) r[i] = drandom(&irand);
