#!/usr/bin/perl
 ...
 sub collect_data
 {
  ...
  for my $i (0..$#urls)
  {
   ...
   my $filename = "$TMP/" . "nec_$i" . "_$proc_num" . ".html"; my $html;
   unless ( &fetch_URL($urls[$i], $timeout, $filename, 1, $get_images[$i], 
                       $debug, $proc_num) )
    {
     ...
     (my $r_text, my $r_links) = &parse_HTML($html, $urls[$i]);

     #*-- START: Customization here ----
     my @links = ();
     foreach my $link (keys %$r_links)
      { next unless ( &get_site($link) =~ /story.news.yahoo.com/i &&
                      $link =~ /tmpl=story/i);
        push (@links, $link); }
     #*-- END:   Customization here ----

     #*-- populate the ne_articles table
     print OUT "Found ", scalar @links, " story links in page\n" if ($debug);
     ...
     foreach my $link (@links)
      {
       ...
       $command = <<"EOF";
        ...
        nec_local_url = '', nec_abstract = '', nei_size = 0,
...
EOF
  ...
  #*-- START: Customization here ---
  my $s_tag = '<!-- TextStart -->'; my $e_tag = '<!-- TextEnd -->';
  my $s_font = '<font face=arial size=-1>'; my $e_font = '</font>';
  #*-- END:   Customization here ---
  ...
  for my $i (0..$#ids)
  {
   ...
   unless ( &fetch_URL($urls[$i], $timeout, $filename, 1, 0, 
                                  $debug, $proc_num) )
    {
     ...
     $size = length($html);
     $html =~ s/^.*$s_tag(.*)$e_tag.*$/$1/s;
     $html =~ s/$s_font(.*?)$e_font.*?$s_font/$1$s_font/si;
     if ( length($html) < 10 )
...
EOF
 ...
 sub gen_summary
 {
  ...
  for my $i (0..$#ids)
   {
    next unless ( ($i % $num_procs) == $proc_num); 
    print OUT "Computing summary for $ids[$i]\n" if ($debug);
    (undef, $db_msg) = $dbh->execute_stmt("lock tables ne_articles read");
    ...
    #*-- get a summary with 2 full sentences without highlighting
    my $q_abstract = $dbh->quote(
               &summary(\$text, 'news', 'Full', $dbh, 0, 2, $hash_refs) );

    (undef, $db_msg) = $dbh->execute_stmt
                       ("lock tables ne_articles write, ne_index write");
    $command = "update ne_articles set nec_abstract = $q_abstract " .
               "where nei_nid = $ids[$i]";
    (undef, $db_msg) = $dbh->execute_stmt($command);
    &quit("Command: $command failed --> $db_msg") if ($db_msg);
