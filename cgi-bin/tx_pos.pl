#!/usr/bin/perl
 ...
 #*- tx_pos.pl
 #*-  
 #*-   Summary: Extract POS tags and change rules
 #*----------------------------------------------------------------
 ...
 use vars (
  ...
  '$FORM',		#*-- name of form
  '@tags',		#*-- tags for words in the text box
  '@words',		#*-- list of words in the text box
  '%in',		#*-- hash for form fields
  '$e_session',		#*-- URL encoded session
  '$body_html',		#*-- HTML for body of page
  '$tbox',		#*-- variable for text box
  '$stat_msg',		#*-- status message for page
  '$buttons'		#*-- buttons at the end of the page
  );
 ...
 &handle_pos()    if ($in{opshun} eq 'Extract POS');
 ...
 sub handle_new
 {
  ...
  $buttons = "<td colspan=2 align=center><input type=submit name=opshun "
           . "value='Extract POS'> &nbsp; &nbsp; <input type=submit "
 ...
 sub handle_pos
 {
  my ($rw, $rl, $rt) = &pos_tagger(\$in{tbox}, $dbh, '', 1);
  @words = @$rw; @tags = @$rt;
  ...
  for my $i (0..($in{num_tags}-1))
   {
    ...
    #*-- create the word forward and word backward rules
    my $tag = (($i-1) < 0) ? 'u': $tags[$i-1];
    my $qword = $dbh->quote(lc($words[$i]));
    $command = <<"EOF";
      replace into co_rulepos set ruc_type = 'WF',
              ruc_wtag = '$in{"w_$i"}', rui_frequency = 1.0,
              ruc_part1 = '$tag', ruc_part2 = $qword
EOF

    $dbh->execute_stmt($command);
    &quit("$command failed: $db_msg") if ($db_msg);

    $tag = ($i == ($in{num_tags}-1) ) ? 'u': $tags[$i+1];
    $command = <<"EOF";
      replace into co_rulepos set ruc_type = 'WB',
              ruc_wtag = '$in{"w_$i"}', rui_frequency = 1.0,
              ruc_part1 = $qword, ruc_part2 = '$tag'
EOF
    $dbh->execute_stmt($command);
    &quit("$command failed: $db_msg") if ($db_msg);
   }
 ...
 #*---------------------------------------------------------------
 #*- build the pos table html
 #*---------------------------------------------------------------
 sub build_pos_table
 {

  #*-- tags and associated description
  my %tagd = qw/a adj. c con. d det. i int. n nou. o pre. p pro.
                r adv. u pun. v ver./;
  $tbox = <<"EOF";
  <tr><td colspan=2>
  ...
  <tr><td colspan=2>
   <table border=0 cellspacing=2 cellpadding=0>
    <tr> <td> <font size=+1 color=darkred> Token </font> </td>
         <td align=center><font size=+1 color=darkred> Part of 
         Speech Tag </font></td></tr>
EOF
 ...
 sub head_html
 {
  my $te_image   = "$ICON_DIR" . "pos.jpg";
  my $anchor = "$CGI_DIR/co_login.pl?session=$e_session";

  print "Content-type: text/html\n\n";
  print << "EOF";
   <html>
    <head> <title> Parts of Speech Tagger </title> </head>
